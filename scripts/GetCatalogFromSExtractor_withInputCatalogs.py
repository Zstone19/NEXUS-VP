...
def combine_refstack_nexus(diffdat_sub, diffdat_sfft, nexdat_edr, nexdat_refstack, nexdat_scistack, band='F200W', matching_radius=.3):
    ...
    cols_in_nex_refstack = ['ALPHA_J2000', 'DELTA_J2000', 'FLUX_AUTO', 'FLUXERR_AUTO', 'FLUX_APER', 'FLUXERR_APER', 'ELONGATION', 'FLAGS', 'IMAFLAGS_ISO', 'CLASS_STAR', 'ISOAREA_IMAGE']
    cols_in_nex_scistack = cols_in_nex_refstack.copy()
    cols_out_nex_refstack = [x + '_REFStack' for x in ['RA', 'DEC', 'FLUX_AUTO', 'FLUXERR_AUTO', 'FLUX_APER', 'FLUXERR_APER', 'ELONGATION', 'FLAGS', 'IMAFLAGS_ISO', 'CLASS_STAR', 'ISOAREA_IMAGE']]
    cols_out_nex_scistack = [ x + '_SCIStack' for x in ['RA', 'DEC', 'FLUX_AUTO', 'FLUXERR_AUTO', 'FLUX_APER', 'FLUXERR_APER', 'ELONGATION', 'FLAGS', 'IMAFLAGS_ISO', 'CLASS_STAR', 'ISOAREA_IMAGE']]
    ...
    for i, ind in enumerate(ap_inds):
        if ind == 0:
            suffix = ''
        else:
            suffix = '_{}'.format(ind)
...
indir = '/data6/stone28/nexus/inputsource_combined_nexus_{}/output_pos/'.format(name_prefix)
cat_nexus_widestack_f200w = Table.read(indir + 'nexus_F200W_ref_stack.cat', format='ascii.sextractor')
cat_nexus_widestack_f444w = Table.read(indir + 'nexus_F444W_ref_stack.cat', format='ascii.sextractor')
cat_nexus_deepstack_f200w = Table.read(indir + 'nexus_F200W_sci_stack.cat', format='ascii.sextractor')
cat_nexus_deepstack_f444w = Table.read(indir + 'nexus_F444W_sci_stack.cat', format='ascii.sextractor')
...
bad_mask_f200w_1 = submask_f200w[yvals, xvals] #Use sub mask for everything
...
bad_mask_f444w_1 = submask_f444w[yvals, xvals] #Use sub mask for everything
