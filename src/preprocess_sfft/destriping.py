...
def get_sourcemask_indiv(maindir, fname_in, imname, logger, overwrite=False):    
    #fname_in should be compressed
    
    outdir = maindir + 'preprocess/stripe/'
    tmpdir = maindir + 'preprocess/tmp/'
    mask_fname = outdir + '{}_noisechisel_mask.fits'.format(imname)
    log_fname = outdir + '{}_noisechisel_mask.log'.format(imname)
    ...
    #Run destriping script and wait until completion
    output = subprocess.run(['astnoisechisel', fname_in, '-o', mask_fname, '-h', '1', '-N', '1', '--oneelempertile', '--rawoutput'], check=True, capture_output=True).stdout

    #Log NoiseChisel output
    for line in output.decode('utf-8').split('\n'):
        logger.info(line)
    
    logger.info('Compressing NoiseChisel mask')
    output = subprocess.run(['gzip', '--best', mask_fname], check=True, capture_output=True).stdout
    for line in output.decode('utf-8').split('\n'):
        logger.info(line)

    return mask_fname + '.gz'
...
def measure_striping(fname_in, fname_out, fname_out_v, fname_out_h, logger, mask_fname=None, thresh=None, apply_flat=True, mask_sources=True, save_patterns=False):
    ...
    Args:
        fname_in (str): input striped image filename, including full relative path
    ...
    # mask out sources
    if mask_sources and (mask_fname is not None):
        logger.info('Using existing source mask %s'%mask_fname)
        seg = fits.getdata(mask_fname, ext=2)
    ...
    logger.info('Image median (unmasked and DQ==0): %f'%(median_image))
    try:
        ...
        logger.info('Fit pedestal: %f'%pedestal)
