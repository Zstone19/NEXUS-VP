...
def make_segmap(f, seg_from_lw=True, sigma=0.8, npixels=10, dilate_segmap=5, save_segmap=False):
    ...
    outfile = f.replace('.fits.gz', '_seg.fits.gz')
...
def process_file(f, logger, outdir, wisp_dir='./', create_segmap=True, seg_from_lw=True, sigma=0.8, npixels=10, dilate_segmap=5,
    ...
    logger.info('Starting DEWISP for {}'.format(f))
    ...
    file_type = 'RATE' #f.split('_')[-1].replace('.fits', '').upper()  # CAL or RATE
    ...
    results = subtract_wisp(f, wisp_data=wisp_data, outdir=outdir, segmap_data=segmap_data, sub_wisp=sub_wisp, 
                            ...
                            save_model=save_model, plot=plot, show_plot=show_plot, suffix=suffix, logger=logger)
    logger.info('Processing complete for {}'.format(f))
    
    logger.info('Finished DEWISP for {}'.format(f))

# -----------------------------------------------------------------------------

def process_files(files, loggers, outdir, nproc=6, **kwargs):
    ...
    mask = np.array([True if any(substring in f for substring in ['nrca3', 'nrca4', 'nrcb3', 'nrcb4']) else False for f in files])
    
    files_in = []
    loggers_in = []
    for i, l in enumerate(loggers):
        if mask[i]:
            files_in.append(files[i])
            loggers_in.append(l)
    
    print('Found {} relevant input files.'.format(len(files_in)))
    
    # Proess the files
    process_file_partial = partial(process_file, outdir=outdir, **kwargs)
    pool = WorkerPool(n_jobs=nproc)
    results = pool.map(process_file_partial, zip(files_in, loggers_in), iterable_len=len(files_in),
                       progress_bar=True, progress_bar_style='rich')
    pool.join()

# -----------------------------------------------------------------------------

def subtract_wisp(f, wisp_data, outdir, segmap_data=None, sub_wisp=True, gauss_smooth_wisp=False, gauss_stddev=3.0, scale_wisp=True,
                  scale_method='mad', poly_degree=5, factor_min=0.0, factor_max=2.0, factor_step=0.01, min_wisp=None, 
                  flag_wisp_thresh=None, dq_val=1, correct_rows=True, correct_cols=False, save_data=True, save_model=True, plot=True, 
                  show_plot=False, suffix='_wisp', logger=None):
    ...
    if logger is None:
        ...
        print_funcw = logger.warning

    # Get the input data
    logger.info('Applying wisp template to {}'.format(f))
    h = fits.open(f)
    data = h['SCI'].data
    dq = h['DQ'].data
    if segmap_data is None:
        logger.info('Warning: No segmap data given for {}. Assuming no sources.'.format(f))
    ...
    if scale_wisp:
        ...
        logger.info('Using best wisp scale factor ({:.3f}) with minimum MAD ({:.3f})'.format(factor, np.min(residuals)))
    ...
    if save_data:
        ...
        time = datetime.now()
        stepdescription = 'Removed wisp; %s'%time.strftime('%Y-%m-%d %H:%M:%S')
        h[0].header['HISTORY'] = stepdescription        
        
        f_out = outdir + os.path.basename(f).replace('_rate.fits.gz', '{}.fits.gz'.format(suffix))
        h.writeto(f_out, overwrite=True)
        logger.info('Saved de-wisped data to file {}'.format(f_out))

    if save_model:
        f_out = outdir + os.path.basename(f).replace('_rate.fits.gz', '{}_model.fits.gz'.format(suffix))
        fits.writeto(f_out, wisp_model, overwrite=True)
        logger.info('Saved wisp model to file {}'.format(f_out))
    h.close()
    ...
    if plot:
        ...
        f_out = outdir + os.path.basename(f).replace('_rate.fits.gz', '{}_plots.png'.format(suffix))
        fig.savefig(f_out, dpi=100, bbox_inches='tight')
        if not show_plot:
            plt.close()
            
        logger.info('Saved diagnostic plot to file {}'.format(f_out))
