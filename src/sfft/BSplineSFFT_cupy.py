...
def make_array(arr_in, ngpu=1):

    if ngpu == 1:
        return cp.array(arr_in)
    else:        
        from cupyx.distributed.array import distributed_array
        
        i_arr = np.linspace(0, arr_in.shape[0], ngpu+1, dtype=int)
        
        chunkmap = {}
        for i in range(ngpu):
            chunkmap[i] = ( slice(i_arr[i], i_arr[i+1]), slice(None, None) )
            
        try:
            arr_GPU = distributed_array(arr_in, chunkmap)
        except:
            arr_GPU = cp.array(arr_in)

        return arr_GPU
    
    

def fft2_multigpu(arr, ngpu, fft_type='C2C'):
    ...
    if fft_type == 'C2C':
        ...
        arr_in = arr.copy().astype(np.complex64)
    if fft_type == 'C2R':
        fft_type = cufft.CUFFT_C2R
        arr_in = arr.copy().astype(np.complex64)
        
    if fft_type == 'R2R':
        fft_type = cufft.CUFFT_R2R
        arr_in = arr.copy()
    if fft_type == 'R2C':
        fft_type = cufft.CUFFTR2C
        arr_in = arr.copy()
    ...
    #FFT 1 (ROWS)
    arr_out_A = np.empty_like(arr_in)
    plan_A = cufft.Plan1d(shape[0], fft_type, shape[1], devices=gpu_list )
    plan_A.fft(arr_in, arr_out_A, cufft.CUFFT_FORWARD)
    
    #FFT 2 (COLS)
    arr_B = arr.T.copy()
    for i in range(shape[1]):
        arr_B[i] = arr_out_A[:,i].copy()
        
    arr_out_B = np.empty_like(arr_B) 
    plan_B = cufft.Plan1d(shape[1], fft_type, shape[0], devices=gpu_list )
    plan_B.fft(arr_B, arr_out_B, cufft.CUFFT_FORWARD)
...
def ifft2_multigpu(arr, ngpu, fft_type='C2C'):
    gpu_list = list(range(ngpu))
    mempool = cp.get_default_memory_pool()
    pinned_mempool = cp.get_default_pinned_memory_pool()
    ...
    arr_out_A = np.empty_like(arr_in)
    plan_A = cufft.Plan1d(shape[0], fft_type, shape[1], devices=gpu_list )
    plan_A.fft(arr_in, arr_out_A, cufft.CUFFT_BACKWARD)
    ...
    arr_out_B = np.empty_like(arr_B) 
    plan_B = cufft.Plan1d(shape[1], fft_type, shape[0], devices=gpu_list )
    plan_B.fft(arr_B, arr_out_B, cufft.CUFFT_BACKWARD)
...
class SingleSFFTConfigure_Cupy:
    ...
    def SSCC(NX, NY, KerHW=8, KerSpType='Polynomial', KerSpDegree=2, KerIntKnotX=[], KerIntKnotY=[], \
        ...
        LAMBDA_REGULARIZE=1e-6, MAX_THREADS_PER_BLOCK=8, MINIMIZE_GPU_MEMORY_USAGE=False, logger=None, VERBOSE_LEVEL=2):
        ...
        if VERBOSE_LEVEL in [1, 2]:
            logger.info('--//--//--//--//-- TRIGGER SFFT COMPILATION [Cupy] --//--//--//--//-- ')
...
class ElementalSFFTSubtract_Cupy:
    ...
    def ESSC(PixA_I, PixA_J, SFFTConfig, SFFTSolution=None, Subtract=False, \
        logger=None, ngpu=1, VERBOSE_LEVEL=2):
        
        import cupyx.scipy.linalg as cpx_linalg
        import scipy.linalg as linalg
        
        make_array_gpu = partial(make_array, ngpu=ngpu)
        
        if ngpu > 1:
            from cupyx.distributed.array import matmul
            import cupy.cuda.cufft as cufft
            
            fft_func = partial(fft2_multigpu, ngpu=ngpu)
            ifft_func = partial(ifft2_multigpu, ngpu=ngpu)
        ...
        if VERBOSE_LEVEL in [1, 2]:
            logger.info('--//--//--//--//-- TRIGGER SFFT SUBTRACTION [Cupy] --//--//--//--//-- ')
        ...
        REF_pq_GPU = make_array_gpu(REF_pq)
        REF_ij_GPU = make_array_gpu(REF_ij)
        REF_ab_GPU = make_array_gpu(REF_ab)
        ...
        if SCALING_MODE == 'SEPARATE-VARYING':
            ...
            if ScaSpType == 'Polynomial':
                ...
                ScaREF_ij_GPU = make_array_gpu(ScaREF_ij)
        ...
        SREF_iji0j0_GPU = make_array_gpu(SREF_iji0j0)
        SREF_pqp0q0_GPU = make_array_gpu(SREF_pqp0q0)
        SREF_ijpq_GPU = make_array_gpu(SREF_ijpq)
        SREF_pqij_GPU = make_array_gpu(SREF_pqij)
        SREF_ijab_GPU = make_array_gpu(SREF_ijab)

        # * Indices related to Constant Scaling Case
        ij00 = np.arange(w0 * L1 + w1, Fijab, Fab).astype(np.int32)
        ij00_GPU = make_array_gpu(ij00)

        t0 = time.time()
        # * Read input images as C-order arrays
        if not PixA_I.flags['C_CONTIGUOUS']:
            PixA_I = np.ascontiguousarray(PixA_I, np.float64)
            PixA_I_GPU = make_array_gpu(PixA_I)
        else: PixA_I_GPU = make_array_gpu(PixA_I.astype(np.float64))
        
        if not PixA_J.flags['C_CONTIGUOUS']:
            PixA_J = np.ascontiguousarray(PixA_J, np.float64)
            PixA_J_GPU = make_array_gpu(PixA_J)
        else: PixA_J_GPU = make_array_gpu(PixA_J.astype(np.float64))
        dt0 = time.time() - t0
        ...
        t1 = time.time()
        PixA_X_GPU = make_array_gpu(  np.zeros((N0, N1), dtype=np.int32)  )     # row index, [0, N0)
        PixA_Y_GPU = make_array_gpu(  np.zeros((N0, N1), dtype=np.int32)  )     # column index, [0, N1)
        PixA_CX_GPU = make_array_gpu(  np.zeros((N0, N1), dtype=np.float64)  )   # coordinate.x
        PixA_CY_GPU = make_array_gpu(  np.zeros((N0, N1), dtype=np.float64)  )   # coordinate.y 
        ...
        # <*****> produce Iij <*****> #
        SPixA_Iij_GPU = make_array_gpu(  np.zeros((Fij, N0, N1), dtype=np.float64)  )
        if KerSpType == 'Polynomial':
        ...
        if KerSpType == 'B-Spline':
            ...
            KerSplBasisX_GPU = make_array_gpu(KerSplBasisX)
            KerSplBasisY_GPU = make_array_gpu(KerSplBasisY)
        ...
        if SCALING_MODE == 'SEPARATE-VARYING':
            ...
            if ScaSpType == 'Polynomial':
                ScaSPixA_Iij_GPU = make_array_gpu(  np.zeros((Fij, N0, N1), dtype=np.float64)  )
            ...
            if ScaSpType == 'B-Spline':
                ...
                ScaSplBasisX_GPU = make_array_gpu(ScaSplBasisX)
                ScaSplBasisY_GPU = make_array_gpu(ScaSplBasisY)
        ...
        # <*****> produce Tpq <*****> #
        SPixA_Tpq_GPU = make_array_gpu(  np.zeros((Fpq, N0, N1), dtype=np.float64)  )
        if BkgSpType == 'Polynomial':
        ...
        if BkgSpType == 'B-Spline':
            ...
            BkgSplBasisX_GPU = make_array_gpu(BkgSplBasisX)
            BkgSplBasisY_GPU = make_array_gpu(BkgSplBasisY)
        ...
        if ngpu > 1:
            ...
            PixA_FJ[:, :] = cp.asnumpy(PixA_J_GPU).astype(np.complex128)
            PixA_FJ[:, :] = fft_func(PixA_FJ)
            PixA_FJ[:, :] *= SCALE

            SPixA_FIij = np.empty((Fij, N0, N1), dtype=np.complex128)
            SPixA_FIij[:, :, :] = cp.asnumpy(SPixA_Iij_GPU).astype(np.complex128)
            for k in range(Fij): 
                SPixA_FIij[k: k+1] = fft_func(SPixA_FIij[k: k+1])
            SPixA_FIij[:, :] *= SCALE

            SPixA_FTpq = np.empty((Fpq, N0, N1), dtype=np.complex128)
            SPixA_FTpq[:, :, :] = cp.asnumpy(SPixA_Tpq).astype(np.complex128)
            for k in range(Fpq): 
                SPixA_FTpq[k: k+1] = fft_func(SPixA_FTpq[k: k+1])
            SPixA_FTpq[:, :] *= SCALE
            ...
            PixA_FJ_GPU = make_array_gpu(PixA_FJ)
            SPixA_FIij_GPU = make_array_gpu(SPixA_FIij)
            SPixA_FTpq_GPU = make_array_gpu(SPixA_FTpq)
            del PixA_FJ
            del SPixA_FIij
            del SPixA_FTpq

        else:
            PixA_FJ_GPU = make_array_gpu( np.empty((N0, N1), dtype=np.complex128) )
            PixA_FJ_GPU[:, :] = PixA_J_GPU.astype(np.complex128)
            PixA_FJ_GPU[:, :] = fft_func(PixA_FJ_GPU)
            PixA_FJ_GPU[:, :] *= SCALE

            SPixA_FIij_GPU = make_array_gpu(  np.empty((Fij, N0, N1), dtype=np.complex128)  )
            SPixA_FIij_GPU[:, :, :] = SPixA_Iij_GPU.astype(np.complex128)
            for k in range(Fij): 
                SPixA_FIij_GPU[k: k+1] = fft_func(SPixA_FIij_GPU[k: k+1])
            SPixA_FIij_GPU[:, :] *= SCALE

            SPixA_FTpq_GPU = make_array_gpu(  np.empty((Fpq, N0, N1), dtype=np.complex128)  )
            SPixA_FTpq_GPU[:, :, :] = SPixA_Tpq_GPU.astype(np.complex128)
            for k in range(Fpq): 
                SPixA_FTpq_GPU[k: k+1] = fft_func(SPixA_FTpq_GPU[k: k+1])
        ...
        if SCALING_MODE == 'SEPARATE-VARYING':
            ...
            if ngpu > 1:
                ...
                ScaSPixA_FIij[:, :, :] = cp.asnumpy(ScaSPixA_Iij_GPU).astype(np.complex128)
                for k in range(ScaFij):
                    ScaSPixA_FIij[k: k+1] = fft_func(ScaSPixA_FIij[k: k+1])
                ScaSPixA_FIij[:, :] *= SCALE
                
                ScaSPixA_FIij_GPU = make_array_gpu(ScaSPixA_FIij)
                del ScaSPixA_FIij

            else:
                ScaSPixA_FIij_GPU = make_array_gpu(  np.empty((Fij, N0, N1), dtype=np.complex128)  )
                ScaSPixA_FIij_GPU[:, :, :] = ScaSPixA_Iij_GPU.astype(np.complex128)
                for k in range(ScaFij):
                    ScaSPixA_FIij_GPU[k: k+1] = fft_func(ScaSPixA_FIij_GPU[k: k+1])
        ...
        if VERBOSE_LEVEL in [1, 2]:
            logger.info('MeLOn CheckPoint: SFFT-SUBTRACTION Preliminary Steps takes [%.4fs]' %dta)
        ...
        if SFFTSolution is not None:
            ...
            Solution_GPU = make_array_gpu(Solution.astype(np.float64))
        ...
        if SFFTSolution is None:            
            ...
            LHMAT_GPU = make_array_gpu(  np.empty((NEQ, NEQ), dtype=np.float64)  )
            RHb_GPU = make_array_gpu(  np.empty(NEQ, dtype=np.float64)  )
            ...
            if SCALING_MODE in ['ENTANGLED', 'SEPARATE-CONSTANT']:
                ...
                if not MINIMIZE_GPU_MEMORY_USAGE:
                    ...
                    _func = _module.get_function('kmain')
                    HpOMG_GPU = make_array_gpu(  np.empty((FOMG, N0, N1), dtype=np.complex128)  )
                    _func(args=(SREF_iji0j0_GPU, SPixA_FIij_GPU, SPixA_CFIij_GPU, HpOMG_GPU), \
                        block=TpB_PIX, grid=BpG_PIX)

                    # b. PreOMG = SCALE * Re[DFT(HpOMG)]
                    if ngpu > 1:
                        HpOMG = cp.asnumpy(HpOMG_GPU).astype(np.complex128)
                        for k in range(FOMG):
                            HpOMG[k: k+1] = fft_func(HpOMG[k: k+1])
                        HpOMG *= SCALE
                        
                        HpOMG_GPU = make_array_gpu(HpOMG)
                        del HpOMG
                    ...
                    PreOMG_GPU = make_array_gpu(  np.empty((FOMG, N0, N1), dtype=np.float64)  )
                    ...
                    _func = _module.get_function('kmain')
                    HpGAM_GPU = make_array_gpu(  np.empty((FGAM, N0, N1), dtype=np.complex128)  )
                    _func(args=(SREF_ijpq_GPU, SPixA_FIij_GPU, SPixA_CFTpq_GPU, HpGAM_GPU), \
                        block=TpB_PIX, grid=BpG_PIX)

                    # b. PreGAM = 1 * Re[DFT(HpGAM)]
                    if ngpu > 1:
                        HpGAM = cp.asnumpy(HpGAM_GPU).astype(np.complex128)
                        for k in range(FGAM):
                            HpGAM[k: k+1] = fft_func(HpGAM[k: k+1])
                        HpGAM *= SCALE
                        
                        HpGAM_GPU = make_array_gpu(HpGAM)
                        del HpGAM
                    ...
                    PreGAM_GPU = make_array_gpu(  np.empty((FGAM, N0, N1), dtype=np.float64)  )
                    ...
                    _func = _module.get_function('kmain')
                    HpPSI_GPU = make_array_gpu(  np.empty((FPSI, N0, N1), dtype=np.complex128)  )
                    ...
                    PrePSI_GPU = make_array_gpu(  np.empty((FPSI, N0, N1), dtype=np.float64)  )
                    ...
                    _func = _module.get_function('kmain')
                    HpPHI_GPU = make_array_gpu(  np.empty((FPHI, N0, N1), dtype=np.complex128)  )
                    ...
                    PrePHI_GPU = make_array_gpu(  np.empty((FPHI, N0, N1), dtype=np.float64)  )
                ...
                if MINIMIZE_GPU_MEMORY_USAGE:
                    ...
                    for cIdx in range(FOMG):
                        ...
                        _module = SFFTModule_dict['HadProd_OMG']
                        _func = _module.get_function('kmain')
                        cHpOMG_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        ...
                        cPreOMG_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                    ...
                    for cIdx in range(FGAM):
                        ...
                        _module = SFFTModule_dict['HadProd_GAM']
                        _func = _module.get_function('kmain')
                        cHpGAM_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        ...
                        cPreGAM_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                    ...
                    for cIdx in range(FPSI):
                        ...
                        _module = SFFTModule_dict['HadProd_PSI']
                        _func = _module.get_function('kmain')
                        cHpPSI_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        ...
                        cPrePSI_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                    ...
                    for cIdx in range(FPHI):
                        ...
                        _module = SFFTModule_dict['HadProd_PHI']
                        _func = _module.get_function('kmain')
                        cHpPHI_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        ...
                        cPrePHI_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                ...
                _func = _module.get_function('kmain')
                HpTHE_GPU = make_array_gpu(  np.empty((FTHE, N0, N1), dtype=np.complex128)  )
                ...
                _func = _module.get_function('kmain')
                HpDEL_GPU = make_array_gpu(  np.empty((FDEL, N0, N1), dtype=np.complex128)  )
                ...
                PreTHE_GPU = make_array_gpu(  np.empty((FTHE, N0, N1), dtype=np.float64)  )
                PreTHE_GPU[:, :, :] = HpTHE_GPU.real
                del HpTHE_GPU
                
                PreDEL_GPU = make_array_gpu(  np.empty((FDEL, N0, N1), dtype=np.float64)  )
            ...
            if SCALING_MODE == 'SEPARATE-VARYING':
                ...
                if MINIMIZE_GPU_MEMORY_USAGE:
                    ...
                    for cIdx in range(FOMG):    
                        ...
                        _func = _module.get_function('kmain')
                        cHpOMG11_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_iji0j0_GPU, SPixA_FIij_GPU, SPixA_CFIij_GPU, cIdx, cHpOMG11_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)
                        
                        # b11. PreOMG11 = SCALE * Re[DFT(HpOMG11)]
                        if ngpu > 1:
                            cHpOMG11 = cp.asnumpy(cHpOMG11_GPU).astype(np.complex128)
                            cHpOMG11[:, :] = fft_func(cHpOMG11)
                            cHpOMG11[:, :] *= SCALE
                            
                            cHpOMG11_GPU = make_array_gpu(cHpOMG11)
                            del cHpOMG11
                        ...
                        cPreOMG11_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                        ...
                        _func = _module.get_function('kmain')
                        cHpOMG01_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_iji0j0_GPU, ScaSPixA_FIij_GPU, SPixA_CFIij_GPU, cIdx, cHpOMG01_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)
                        
                        # b01. PreOMG01 = SCALE * Re[DFT(HpOMG01)]
                        if ngpu > 1:
                            cHpOMG01 = cp.asnumpy(cHpOMG01_GPU).astype(np.complex128)
                            cHpOMG01[:, :] = fft_func(cHpOMG01)
                            cHpOMG01[:, :] *= SCALE
                            
                            cHpOMG01_GPU = make_array_gpu(cHpOMG01)
                            del cHpOMG01
                        ...
                        cPreOMG01_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                        ...
                        _func = _module.get_function('kmain')
                        cHpOMG10_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_iji0j0_GPU, SPixA_FIij_GPU, ScaSPixA_CFIij_GPU, cIdx, cHpOMG10_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)
                        
                        # b10. PreOMG10 = SCALE * Re[DFT(HpOMG10)]
                        if ngpu > 1:
                            cHpOMG10 = cp.asnumpy(cHpOMG10_GPU).astype(np.complex128)
                            cHpOMG10[:, :] = fft_func(cHpOMG10)
                            cHpOMG10[:, :] *= SCALE
                            
                            cHpOMG10_GPU = make_array_gpu(cHpOMG10)
                            del cHpOMG10
                        ...
                        cPreOMG10_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                        ...
                        _func = _module.get_function('kmain')
                        cHpOMG00_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_iji0j0_GPU, ScaSPixA_FIij_GPU, ScaSPixA_CFIij_GPU, cIdx, cHpOMG00_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)
                        
                        # b00. PreOMG00 = SCALE * Re[DFT(HpOMG00)]
                        if ngpu > 1:
                            cHpOMG00 = cp.asnumpy(cHpOMG00_GPU).astype(np.complex128)
                            cHpOMG00[:, :] = fft_func(cHpOMG00)
                            cHpOMG00[:, :] *= SCALE
                            
                            cHpOMG00_GPU = make_array_gpu(cHpOMG00)
                            del cHpOMG00
                        ...
                        cPreOMG00_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                    ...
                    for cIdx in range(FGAM):
                        ...
                        _func = _module.get_function('kmain')
                        cHpGAM1_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_ijpq_GPU, SPixA_FIij_GPU, SPixA_CFTpq_GPU, cIdx, cHpGAM1_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)

                        # b1. PreGAM1 = 1 * Re[DFT(HpGAM1)]
                        if ngpu > 1:
                            cHpGAM1 = cp.asnumpy(cHpGAM1_GPU).astype(np.complex128)
                            cHpGAM1[:, :] = fft_func(cHpGAM1)
                            cHpGAM1[:, :] *= SCALE
                            
                            cHpGAM1_GPU = make_array_gpu(cHpGAM1)
                            del cHpGAM1
                        else:
                            cHpGAM1_GPU = cp.fft.fft2(cHpGAM1_GPU)
                            cHpGAM1_GPU *= SCALE

                        cPreGAM1_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                        ...
                        _func = _module.get_function('kmain')
                        cHpGAM0_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_ijpq_GPU, ScaSPixA_FIij_GPU, SPixA_CFTpq_GPU, cIdx, cHpGAM0_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)

                        # b0. PreGAM0 = 1 * Re[DFT(HpGAM0)]
                        if ngpu > 1:
                            cHpGAM0 = cp.asnumpy(cHpGAM0_GPU).astype(np.complex128)
                            cHpGAM0[:, :] = fft_func(cHpGAM0)
                            cHpGAM0[:, :] *= SCALE
                            
                            cHpGAM0_GPU = make_array_gpu(cHpGAM0)
                            del cHpGAM0
                        ...
                        cPreGAM0_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                    ...
                    for cIdx in range(FPSI):
                        ...
                        _func = _module.get_function('kmain')
                        cHpPSI1_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_pqij_GPU, SPixA_CFIij_GPU, SPixA_FTpq_GPU, cIdx, cHpPSI1_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)

                        # b1. PrePSI1 = 1 * Re[DFT(HpPSI1)]
                        if ngpu > 1:
                            cHpPSI1 = cp.asnumpy(cHpPSI1_GPU).astype(np.complex128)
                            cHpPSI1[:, :] = fft_func(cHpPSI1)
                            cHpPSI1[:, :] *= SCALE
                            
                            cHpPSI1_GPU = make_array_gpu(cHpPSI1)
                            del cHpPSI1
                        ...
                        cPrePSI1_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                        ...
                        _func = _module.get_function('kmain')
                        cHpPSI0_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
                        _func(args=(SREF_pqij_GPU, ScaSPixA_CFIij_GPU, SPixA_FTpq_GPU, cIdx, cHpPSI0_GPU), \
                            block=TpB_PIX, grid=BpG_PIX)

                        # b1. PrePSI0 = 1 * Re[DFT(HpPSI0)]
                        if ngpu > 1:
                            cHpPSI0 = cp.asnumpy(cHpPSI0_GPU).astype(np.complex128)
                            cHpPSI0[:, :] = fft_func(cHpPSI0)
                            cHpPSI0[:, :] *= SCALE
                            
                            cHpPSI0_GPU = make_array_gpu(cHpPSI0)
                            del cHpPSI0
                        ...
                        cPrePSI0_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.float64)  )
                    ...
                    for cIdx in range(FPHI):
                        ...
                        # b. PrePHI = SCALE_L * Re[DFT(HpPHI)]
                        if ngpu > 1:
                            cHpPHI = cp.asnumpy(cHpPHI_GPU).astype(np.complex128)
                            cHpPHI[:, :] = fft_func(cHpPHI)
                            cHpPHI[:, :] *= SCALE
                            
                            cHpPHI_GPU = make_array_gpu(cHpPHI)
                            del cHpPHI
                ...
                _func = _module.get_function('kmain')
                HpTHE1_GPU = make_array_gpu(  np.empty((FTHE, N0, N1), dtype=np.complex128)  )
                ...
                _func = _module.get_function('kmain')
                HpTHE0_GPU = make_array_gpu(  np.empty((FTHE, N0, N1), dtype=np.complex128)  )
                ...
                if ngpu > 1:
                    HpTHE1 = cp.asnumpy(HpTHE1_GPU).astype(np.complex128)
                    for k in range(FTHE):
                        HpTHE1[k: k+1] = fft_func(HpTHE1[k: k+1])
                    HpTHE1[:, :, :] *= SCALE
                    
                    HpTHE1_GPU = make_array_gpu(HpTHE1)
                    del HpTHE1
                else:
                    for k in range(FTHE):
                ...
                PreTHE1_GPU = make_array_gpu(  np.empty((FTHE, N0, N1), dtype=np.float64)  )
                PreTHE1_GPU[:, :, :] = HpTHE1_GPU.real
                del HpTHE1_GPU

                if ngpu > 1:
                    HpTHE0 = cp.asnumpy(HpTHE0_GPU).astype(np.complex128)
                    for k in range(FTHE):
                        HpTHE0[k: k+1] = fft_func(HpTHE0[k: k+1])
                    HpTHE0[:, :, :] *= SCALE
                    
                    HpTHE0_GPU = make_array_gpu(HpTHE0)
                    del HpTHE0
                else:
                    for k in range(FTHE):
                ...
                PreTHE0_GPU = make_array_gpu(  np.empty((FTHE, N0, N1), dtype=np.float64)  )
                PreTHE0_GPU[:, :, :] = HpTHE0_GPU.real
                del HpTHE0_GPU

                if ngpu > 1:
                    HpDEL = cp.asnumpy(HpDEL_GPU).astype(np.complex128)
                    for k in range(FDEL):
                        HpDEL[k: k+1] = fft_func(HpDEL[k: k+1])
                    HpDEL[:, :, :] *= SCALE
                    
                    HpDEL_GPU = make_array_gpu(HpDEL)
                    del HpDEL
                else:
                    for k in range(FDEL):
                        HpDEL_GPU[k: k+1] = cp.fft.fft2(HpDEL_GPU[k: k+1])
            ...
            if REGULARIZE_KERNEL:
                ...
                SPMAT_GPU = make_array_gpu( SPMAT.astype(np.float64) )
                ...
                if SCALING_MODE == 'SEPARATE-VARYING':
                    ...
                    ScaSPMAT_GPU = make_array_gpu( ScaSPMAT.astype(np.float64)  )

                if WEIGHT_REGULARIZE is None:
                    SSTMAT_GPU = matmul(SPMAT_GPU, SPMAT_GPU.T)/NREG   # symmetric
                    if SCALING_MODE == 'SEPARATE-VARYING':
                        CSSTMAT_GPU = matmul(SPMAT_GPU, ScaSPMAT_GPU.T)/NREG     # C: Cross
                        DSSTMAT_GPU = matmul(ScaSPMAT_GPU, ScaSPMAT_GPU.T)/NREG  # D: Double, symmetric
                ...
                if WEIGHT_REGULARIZE is not None:
                    ...
                    WSPMAT_GPU = make_array_gpu(WSPMAT, dtype=np.float64)

                    SSTMAT_GPU = matmul(matmul(SPMAT_GPU, WSPMAT_GPU), SPMAT_GPU.T)   # symmetric
                    if SCALING_MODE == 'SEPARATE-VARYING':
                        CSSTMAT_GPU = matmul(matmul(SPMAT_GPU, WSPMAT_GPU), ScaSPMAT_GPU.T)   # C: Cross
                        DSSTMAT_GPU = matmul(matmul(ScaSPMAT_GPU, WSPMAT_GPU), ScaSPMAT_GPU.T)   # D: Double, symmetric
                ...
                LAPMAT_GPU = make_array_gpu(LAPMAT.astype(np.int32))
                RRF_GPU = make_array_gpu(RRF.astype(np.int32))
                CCF_GPU = make_array_gpu(CCF.astype(np.int32))
                ...
                LTLMAT_GPU = matmul(LAPMAT_GPU.T, LAPMAT_GPU)   # symmetric
                
                # Create iREGMAT
                iREGMAT_GPU = make_array_gpu(  np.zeros((Fab, Fab), dtype=np.int32)  )
                _module = SFFTModule_dict['fill_iregmat']    
                _func = _module.get_function('kmain')
                _func(args=(iREGMAT_GPU, LTLMAT_GPU), block=TpB_iREG, grid=BpG_iREG)

                # Create REGMAT
                REGMAT_GPU = make_array_gpu(  np.zeros((NEQ, NEQ), dtype=np.float64)  )
                if SCALING_MODE in ['ENTANGLED', 'SEPARATE-CONSTANT']:
            ...
            if SCALING_MODE == 'SEPARATE-CONSTANT':
                ...
                LHMAT_tweaked_GPU = make_array_gpu(  np.empty((NEQt, NEQt), dtype=np.float64)  )
                RHb_tweaked_GPU = make_array_gpu(  np.empty(NEQt, dtype=np.float64)  )
                ...
                PresIDX_GPU = make_array_gpu(PresIDX)
            ...
            if SCALING_MODE == 'ENTANGLED' or (SCALING_MODE == 'SEPARATE-VARYING' and NEQt == NEQ):
                ...
                if ngpu > 1:
                    LHMAT = cp.asnumpy(LHMAT_GPU).astype(np.float64)
                    RHb = cp.asnumpy(RHb_GPU).astype(np.float64)
                    
                    Solution = LSSolver(A=LHMAT, b=RHb)
                    Solution_GPU = make_array_gpu(Solution)
                    del LHMAT, RHb
                    del Solution
                else:
            ...
            if SCALING_MODE == 'SEPARATE-CONSTANT':
                if ngpu > 1:
                    LHMAT_tweaked = cp.asnumpy(LHMAT_tweaked_GPU).astype(np.float64)
                    RHb_tweaked = cp.asnumpy(RHb_tweaked_GPU).astype(np.float64)
                    
                    Solution_tweaked = LSSolver(A=LHMAT_tweaked, b=RHb_tweaked)
                    Solution_tweaked_GPU = make_array_gpu(Solution_tweaked)
                    del LHMAT_tweaked, RHb_tweaked
                    del Solution_tweaked
                    
                else:
                    Solution_tweaked_GPU = LSSolver(A_GPU=LHMAT_tweaked_GPU, b_GPU=RHb_tweaked_GPU)
                    
                del LHMAT_tweaked_GPU, RHb_tweaked_GPU
                
                BpG_restore, TpB_restore = GPUManage(NEQt)
                BpG_restore, TpB_restore = (BpG_restore, 1), (TpB_restore, 1, 1)

                if KerSpType == 'Polynomial':
                    Solution_GPU = make_array_gpu(  np.zeros(NEQ, dtype=np.float64)  )
            ...
            if VERBOSE_LEVEL in [1, 2]:
                logger.info('MeLOn CheckPoint: SFFT-SUBTRACTION Establish & Solve Linear System takes [%.4fs]' %dtb)
        ...
        if Subtract:
            ...
            if ngpu > 1:
                PixA_X = cp.asnumpy(PixA_X_GPU).astype(np.float64)
                PixA_Y = cp.asnumpy(PixA_Y_GPU).astype(np.float64)
                
                Wl = np.exp((-2j*np.pi/N0) * PixA_X)    # row index l, [0, N0)
                Wm = np.exp((-2j*np.pi/N1) * PixA_Y)
                
                Wl_GPU = make_array_gpu(Wl)
                Wm_GPU = make_array_gpu(Wm)
                del PixA_X
                del PixA_Y
                del Wl
                del Wm
            ...
            Kab_Wla_GPU = make_array_gpu(  np.empty((L0, N0, N1), dtype=np.complex128)  )
            Kab_Wmb_GPU = make_array_gpu(  np.empty((L1, N0, N1), dtype=np.complex128)  )
            ...
            if SCALING_MODE in ['ENTANGLED', 'SEPARATE-CONSTANT']:
                ...
                _func = _module.get_function('kmain')     
                PixA_FDIFF_GPU = make_array_gpu(  np.empty((N0, N1), dtype=np.complex128)  )
            ...
            del SREF_ijab_GPU, REF_ab_GPU, a_ijab_GPU, b_pq_GPU
            # del SPixA_FIij_GPU, ScaSPixA_FIij_GPU
            del Kab_Wla_GPU, Kab_Wmb_GPU
            del SPixA_FTpq_GPU, PixA_FJ_GPU
            
            # Get Difference & Reconstructed Images
            if ngpu > 1:
                PixA_FDIFF = cp.asnumpy(PixA_FDIFF_GPU).astype(np.complex128)
                PixA_FDIFF[:, :] = fft_func(PixA_FDIFF)
                PixA_FDIFF[:, :] *= SCALE_L
                
                PixA_FDIFF = make_array_gpu(PixA_FDIFF)
            else:
        ...
        if VERBOSE_LEVEL in [1, 2]:
            logger.info('--||--||--||--||-- EXIT SFFT SUBTRACTION [Cupy] --||--||--||--||-- ')
...
class ElementalSFFTSubtract:
    ...
    def ESS(PixA_I, PixA_J, SFFTConfig, SFFTSolution=None, Subtract=False, \
        BACKEND_4SUBTRACT='Cupy', NUM_CPU_THREADS_4SUBTRACT=8, logger=None, ngpu=1, VERBOSE_LEVEL=2):
        ...
        if BACKEND_4SUBTRACT == 'Cupy':
            Solution, PixA_DIFF = ElementalSFFTSubtract_Cupy.ESSC(PixA_I=PixA_I, PixA_J=PixA_J, \
                ...
                logger=logger, ngpu=ngpu, VERBOSE_LEVEL=VERBOSE_LEVEL)
...
class GeneralSFFTSubtract:
    ...
    def GSS(PixA_I, PixA_J, PixA_mI, PixA_mJ, SFFTConfig, ContamMask_I=None, \
        ...
        Solution = ElementalSFFTSubtract.ESS(PixA_I=PixA_mI, PixA_J=PixA_mJ, SFFTConfig=SFFTConfig, \
            ...
            NUM_CPU_THREADS_4SUBTRACT=NUM_CPU_THREADS_4SUBTRACT, logger=logger, ngpu=ngpu, VERBOSE_LEVEL=VERBOSE_LEVEL)[0]
        ...
        PixA_DIFF = ElementalSFFTSubtract.ESS(PixA_I=PixA_I, PixA_J=PixA_J, SFFTConfig=SFFTConfig, \
            ...
            NUM_CPU_THREADS_4SUBTRACT=NUM_CPU_THREADS_4SUBTRACT, logger=logger, ngpu=ngpu, VERBOSE_LEVEL=VERBOSE_LEVEL)[1]
...
class BSpline_Packet:
    ...
    def BSP(FITS_REF, FITS_SCI, FITS_mREF, FITS_mSCI, FITS_DIFF=None, FITS_Solution=None, \
        ...
        NUM_CPU_THREADS_4SUBTRACT=8, logger=None, ngpu=1, VERBOSE_LEVEL=2):
        ...
        * Parameters of Customized SFFT
        ...
        _tmp = GeneralSFFTSubtract.GSS(PixA_I=PixA_I, PixA_J=PixA_J, PixA_mI=PixA_mI, PixA_mJ=PixA_mJ, \
            ...
            NUM_CPU_THREADS_4SUBTRACT=NUM_CPU_THREADS_4SUBTRACT, logger=logger, ngpu=ngpu, VERBOSE_LEVEL=VERBOSE_LEVEL)
...
class BSpline_GridConvolve:
    ...
    def GSVC_GPU(self, CUDA_DEVICE='0', CLEAN_GPU_MEMORY=False, nproc=32):
        ...
        if not self.normalize_kernel:
            self.KerStack = np.array(self.KerStack, dtype=np.float64)
        else:
            sums = np.sum(self.KerStack, axis=(1,2))
            self.KerStack = np.array(self.KerStack / sums[:, np.newaxis, np.newaxis], dtype=np.float64)
        ...
        if self.use_fft:
            for idx in range(Nseg):
                if np.nansum(self.KerStack[idx]) == 0.0:
                    continue
                
                Ker_GPU = cp.array(self.KerStack[idx].astype(np.float64))                
